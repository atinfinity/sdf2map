...
- **heightmap**: グレースケール画像のみ対応。DEM / GeoTIFF 地形は
  警告を出してスキップします。
- **静的スナップショット**: static でないモデルも初期 pose で
  サンプリングします。ジョイント状態はシミュレートせず、`<actor>` は
  仕様として無視します。
- **複数world**: ファイル内の最初の `<world>` のみ処理します
  (NOTE を表示)。
- **plane は有限**: `<plane>` は宣言された `<size>` の範囲のみ
  サンプリングします。SDF の既定値は 1×1 m のため、暗黙の無限地面に
  依存した world では小さなパッチしか生成されません。
- **model:// URI は自動ダウンロード対象外**: ダウンロードされるのは
  完全な Fuel URL (`https://fuel.gazebosim.org/...`) のみ。`model://`
  は `--model-path`・環境変数・キャッシュで解決する必要があります。
- **メッシュ**: 三角形サブメッシュのみサンプリング。strip / fan /
  line / point 形式は警告を出してスキップします。
- **透明判定**は `<transparency>` 要素のみ参照。PBR マテリアルの
  アルファやテクスチャの透過は見ません。
- **占有格子**は occupied / free の2値のみで、unknown (灰色) セルは
  生成しません (ジオメトリが既知のため)。
- **形状のみ**: PCD は XYZ 点のみで、色・強度・法線は含みません。
  極端に薄い/小さい形状は `--density` を上げないと点が疎になる
  ことがあります。
